-- Fail-Safe Curriculum Update
-- This script handles potential duplicates and missing subjects explicitly.

-- 1. Ensure 'Legislación' exists and has correct name (normalized)
INSERT INTO subjects (nombre, codigo, año, numero_materia)
SELECT 'Legislación', 'LEG', 4, 24
WHERE NOT EXISTS (SELECT 1 FROM subjects WHERE nombre ILIKE 'Legislación');

-- 2. Force correct numbers (Using specific updates to avoid unique constraint collisions)
-- We set to temporary negative numbers first to clear the way
UPDATE subjects SET numero_materia = -1 * numero_materia;
...
-- 3. If any remained negative, it means they weren't matched. Fix names manually if needed or delete.
-- For now, we assume names are correct. 
-- BUT, if duplicates existed before (e.g. two 'Probabilidad'), we need to clean them up.
-- We can't easily auto-deduplicate without losing data, but given this is a clean slate setup...
-- If we have duplicates, one will keep negative.


-- 4. Re-apply Dependencies
