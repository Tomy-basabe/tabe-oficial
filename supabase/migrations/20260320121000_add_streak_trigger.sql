-- Migration to officially add the streak update trigger on study_sessions
...
BEGIN
    ...
    ELSIF days_diff > 1 THEN
        ...
        IF current_freezes > 0 THEN
            ...
            WHERE user_id = NEW.user_id;
        ELSE
            -- Break streak and start at 1 for today
            UPDATE public.user_stats
            SET racha_actual = 1,
                updated_at = now()
            WHERE user_id = NEW.user_id;
...
-- Create the trigger
DROP TRIGGER IF EXISTS update_user_streak ON public.study_sessions;
