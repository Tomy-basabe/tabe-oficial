...
const CommandList = forwardRef<CommandListRef, CommandListProps>(
  ({ items, command }, ref) => {
    ...
    if (items.length === 0) {
      return (
        <div className="slash-menu-empty bg-card/95 backdrop-blur-xl border-2 border-border/80 rounded-2xl p-5 shadow-2xl text-center min-w-[280px] animate-in fade-in zoom-in-95 duration-150">
          <div className="w-10 h-10 rounded-xl bg-secondary/80 flex items-center justify-center mx-auto mb-2.5 text-muted-foreground border border-border/60">
            <HelpCircle className="w-5 h-5" />
          </div>
          <p className="text-xs font-bold text-foreground">No se encontraron comandos</p>
          <p className="text-[11px] text-muted-foreground mt-1">Prueba escribiendo /texto, /h1, /color o /tabla</p>
        </div>
      );
    }

    return (
      <div className="slash-menu bg-card/95 backdrop-blur-xl border-2 border-border/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col w-[350px] sm:w-[380px] max-w-[calc(100vw-2rem)] max-h-[420px] animate-in fade-in zoom-in-95 duration-150">
        {/* Scrollable list */}
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-1.5 space-y-3">
          {Object.entries(groupedItems).map(([category, categoryItems]) => (
            <div key={category} className="space-y-0.5">
              <div className="sticky top-0 z-10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-muted-foreground/80 bg-card/95 backdrop-blur-md rounded-lg flex items-center justify-between border-b border-border/30">
                <span>{category}</span>
                <span className="text-[9px] font-medium opacity-60">{categoryItems.length}</span>
              </div>
                ...
                {categoryItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2.5 w-full px-2.5 py-2 text-left rounded-xl transition-all duration-150 group relative select-none",
                        isSelected
                          ? "bg-primary/15 text-foreground border border-primary/35 shadow-xs translate-x-0.5"
                          : "hover:bg-secondary/70 text-foreground/85 border border-transparent"
                      )}
                    >
                      <div
                        className={cn(
                          ...
                          isSelected
                            ? "bg-primary/25 text-primary border border-primary/40 scale-105"
                            : "bg-secondary/80 text-muted-foreground border border-border/40 group-hover:text-foreground group-hover:scale-105"
                        )}
                      >
                        {item.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-xs font-bold leading-tight truncate", isSelected ? "text-foreground" : "text-foreground/90")}>
                          {item.title}
                        </p>
                      ...
                      {item.shortcut && (
                        <span className="shrink-0 text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded-md bg-secondary/80 text-muted-foreground border border-border/50">
                          {item.shortcut}
                        </span>
                      )}
                    </button>
        ...
        <div className="px-3 py-1.5 border-t border-border/40 bg-secondary/30 flex items-center justify-between text-[10px] font-bold text-muted-foreground/75 select-none shrink-0">
          <div className="flex items-center gap-1.5">
            <kbd className="px-1 py-0.5 rounded bg-card border border-border/60 font-mono text-[9px]">↑↓</kbd>
            <span>Navegar</span>
          </div>
          <div className="flex items-center gap-1.5">
            <kbd className="px-1 py-0.5 rounded bg-card border border-border/60 font-mono text-[9px]">↵</kbd>
          ...
          <div className="flex items-center gap-1.5">
            <kbd className="px-1 py-0.5 rounded bg-card border border-border/60 font-mono text-[9px]">Esc</kbd>
            <span>Cerrar</span>
          </div>
        </div>
      </div>
