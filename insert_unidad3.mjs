...
function parseQuestions() {
  const rawText = fs.readFileSync('raw_unidad3.txt', 'utf8');
...
async function run() {
  ...
  console.log("Creating quiz deck Unidad3CD...");
  const { data: deck, error: deckErr } = await supabase
    ...
    .insert({
      ...
      subject_id: null,
      nombre: 'Unidad3CD',
      description: 'Cuestionario Unidad 3',
      is_public: false,
  ...
  console.log(`Done inserting ${questionsData.length} questions for Unidad3CD!`);
}
